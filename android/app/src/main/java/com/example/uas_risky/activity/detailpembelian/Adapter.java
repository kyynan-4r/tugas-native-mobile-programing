...
public class Adapter extends RecyclerView.Adapter<Adapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        // TODO Edit This Scarlet
        TextView pembelianid = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(pembelianid);
        TextView produkid = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(produkid);
        TextView kuririd = Utils.createTextView(context);
        viewHolder.itemCountainer.addView(kuririd);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // TODO Edit This Scarlet
        ((TextView) holder.itemCountainer.getChildAt(0)).setText(String.format("ID Pembelian: %s", data.pembelianid));
        ((TextView) holder.itemCountainer.getChildAt(1)).setText(String.format("ID Produk: %s", data.produkid));
        ((TextView) holder.itemCountainer.getChildAt(2)).setText(String.format("ID Kurir: %s", data.kuririd));
        ((TextView) holder.itemCountainer.getChildAt(3)).setText(String.format("Harga Satuan: %s", data.hargasatuan));
        ((TextView) holder.itemCountainer.getChildAt(4)).setText(String.format("Jumlah: %s", data.jumlah));
        ((TextView) holder.itemCountainer.getChildAt(5)).setText(String.format("Sub Total: %s", data.subtotal));
    ...
    private void showUpdateModal(Model model) {
        ...
        // TODO Edit This Scarlet
        EditText pembelianid = Utils.createEditText(context, "pembelianid");
        pembelianid.setText(model.pembelianid);
        updateContainer.addView(pembelianid);
        EditText produkid = Utils.createEditText(context, "produkid");
        produkid.setText(model.produkid);
        updateContainer.addView(produkid);
        EditText kuririd = Utils.createEditText(context, "kuririd");
        kuririd.setText(model.kuririd);
        updateContainer.addView(kuririd);
        EditText hargasatuan = Utils.createEditText(context, "hargasatuan");
        hargasatuan.setText(model.kuririd);
        ...
        updateButton.setOnClickListener(v -> {
            ...
            if (!Objects.equals(model.pembelianid, pembelianid.getText().toString())) {
                updatedModel.put("pembelianid", pembelianid.getText().toString());
            }
            if (!Objects.equals(model.produkid, produkid.getText().toString())) {
                updatedModel.put("produkid", produkid.getText().toString());
            }
            if (!Objects.equals(model.kuririd, kuririd.getText().toString())) {
                updatedModel.put("kuririd", kuririd.getText().toString());
            }
